...
void kmeans(uint8_t k, cluster* centroides, uint32_t num_pixels, rgb* pixels){	
	...
	for(i = 0; i < k; i++) 
	{
        ...
        // Inicialitzar les variables auxiliars reservant la memòria
        uint32_t * pixels_aux = malloc(num_pixels * sizeof(uint32_t));
        uint32_t * r_aux = malloc(k * sizeof(uint32_t));
        uint32_t * g_aux = malloc(k * sizeof(uint32_t));
        uint32_t * b_aux = malloc(k * sizeof(uint32_t));
	...
	// Alliberem la memòria dels arrays auxiliars
	free(pixels_aux);
    	free(r_aux);
    	free(g_aux);
    	free(b_aux);
